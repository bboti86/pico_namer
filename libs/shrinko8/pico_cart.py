...
def read_cart_from_image(data, path=None, **opts):
    try:
        from media_utils import _pil_module
        _pil_module() # Test if PIL is installed
        ...
        for y in range(height):
            for x in range(width):
                ...
                cart_data.append(byte)

        cart = read_cart_from_rom(cart_data, **opts)
        cart.label = surface_pixels_to_screenshot(pixels, k_label_offset)
        return cart

    except Exception:
        # Fallback to PySDL2 since PIL is missing on Spruce OS
        import sdl2
        import sdl2.sdlimage
        import ctypes
        
        # We need to make sure SDL_image is initialized
        sdl2.sdlimage.IMG_Init(sdl2.sdlimage.IMG_INIT_PNG)
        
        if path:
            surface_ptr = sdl2.sdlimage.IMG_Load(path.encode('utf-8'))
        else:
            rw = sdl2.SDL_RWFromConstMem(data, len(data))
            surface_ptr = sdl2.sdlimage.IMG_Load_RW(rw, 1)
            
        if not surface_ptr:
            throw("Failed to load image via SDL2 fallback")
        ...
        # Ensure format is standardized to read RGBA easily (PICO-8 PNGs are 32bpp)
        # Convert to RGBA32 format if necessary
        formatted_surface_ptr = sdl2.SDL_ConvertSurfaceFormat(surface_ptr, sdl2.SDL_PIXELFORMAT_ABGR8888, 0)
        sdl2.SDL_FreeSurface(surface_ptr)
        
        if not formatted_surface_ptr:
            throw("Failed to convert surface format in SDL2 fallback")
            
        formatted_surface = formatted_surface_ptr.contents
        pixels_ptr = ctypes.cast(formatted_surface.pixels, ctypes.POINTER(ctypes.c_uint32))
        
        cart_data = bytearray()
        for y in range(height):
            for x in range(width):
                pixel = pixels_ptr[y * (formatted_surface.pitch // 4) + x]
                # ABGR8888 in little-endian gives memory order R G B A.
                # Just use GetRGBA to be safe.
                r, g, b, a = ctypes.c_uint8(), ctypes.c_uint8(), ctypes.c_uint8(), ctypes.c_uint8()
                sdl2.SDL_GetRGBA(pixel, formatted_surface.format, ctypes.byref(r), ctypes.byref(g), ctypes.byref(b), ctypes.byref(a))
                
                byte = ((b.value & 3) << 0) | ((g.value & 3) << 2) | ((r.value & 3) << 4) | ((a.value & 3) << 6)
                cart_data.append(byte)
                
        sdl2.SDL_FreeSurface(formatted_surface_ptr)
